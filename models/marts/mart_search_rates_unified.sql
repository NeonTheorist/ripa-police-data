...
base AS (
  SELECT stop_id, perceived_race, stop_month, MAX(row_has_search) AS had_search
  FROM base_raw
  GROUP BY 1,2,3
),
finds AS (
...
SELECT
  ...
  SAFE_DIVIDE(COUNT(DISTINCT IF(b.had_search = 1, b.stop_id, NULL)),
              COUNT(DISTINCT b.stop_id)) AS search_rate,
  COUNT(DISTINCT IF(f.stop_id IS NOT NULL, b.stop_id, NULL)) AS finds,
  SAFE_DIVIDE(COUNT(DISTINCT IF(f.stop_id IS NOT NULL, b.stop_id, NULL)),
              NULLIF(COUNT(DISTINCT IF(b.had_search = 1, b.stop_id, NULL)), 0)) AS yield_rate
FROM base b
